...
import { Button } from "@stayboost/ui";
import { getDictionary } from "@stayboost/i18n";
import { api } from "@/lib/api";

const t = getDictionary().auth.common;

/** Starts the Google OAuth flow by fetching the consent URL and redirecting. */
export function GoogleButton(): React.JSX.Element {
  ...
  async function start(): Promise<void> {
    ...
    try {
      ...
      setLoading(false); // Google not configured / unavailable; stay on the page.
    }
  }
  ...
  return (
    <Button type="button" variant="outline" className="w-full" disabled={loading} onClick={start}>
      <svg aria-hidden viewBox="0 0 24 24" className="size-4">
        ...
        />
        <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.26 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z" />
